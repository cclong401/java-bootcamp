...
public class LibraryService {
    ...
    public void borrowBook() {
        ...
        if (!bookIds.contains(bookInput)) {
            System.out.println("Book not found.");
            return;
        }

        // reject if already borrowed; mark unavailable; put borrowRecords
        if (borrowRecords.containsKey(bookInput)) {
        ...
        if (!memberIds.contains(memberInput)) {
            System.out.println("Member not found.");
            return;
        }

        Book book = findBookById(bookInput);
        book.setAvailable(false);
        borrowRecords.put(bookInput, memberInput);

        // add BorrowRecord to history; bump borrowFrequency; success message
        borrowHistory.add(new BorrowRecord(bookInput, memberInput, LocalDate.now()));
        borrowFrequency.merge(memberInput, 1, Integer::sum);

        System.out.println(book.getTitle() + " borrowed on " + LocalDate.now());
    }
    ...
    public void returnBook() {
        ...
        Book book = findBookById(bookInput);
        borrowRecords.remove(bookInput);
        book.setAvailable(true);

        // set returnDate on latest open BorrowRecord
        borrowHistory.get(borrowHistory.size() - 1).setReturnDate(LocalDate.now());

        System.out.println(book.getTitle() + " returned on " + LocalDate.now());
    }
