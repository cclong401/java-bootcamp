...
public class StudentManager {
    ...
    public void addStudent() {
        // reject when studentCount >= MAX_STUDENTS
        if (studentCount >= MAX_STUDENTS) { System.out.println("FULL"); return; }

        // read studentId (positive int); reject duplicate IDs via findStudentIndex
        int id = 0;
        do {
            System.out.print("Student ID: ");
            int tempId = readPositiveInt();
            if (findStudentIndex(tempId) != -1) {
                System.out.println("ID already in use");
            }
            else id = tempId;
        } while (id == 0);

        // read name, course, marks; store new Student at students[studentCount++]
        String name = readNonEmptyLine("Name: ");
        String course = readNonEmptyLine("Course: ");
        double marks = 0;
        do {
            System.out.print("Marks: ");
            double tempMarks = Double.parseDouble(scanner.nextLine());
            if (tempMarks < 0 || tempMarks > 100 ) {
                System.out.println("Must be between 0 and 100");
            }
            else  marks = tempMarks;
        } while (marks == 0);
    ...
    public void displayStudents() {
        ...
        if (studentCount == 0) System.out.println("No students to display.");
        // otherwise call printStudentTable(students, studentCount, false)
        else printStudentTable(students, studentCount, false);
    }

    public void searchStudent() {
        // read Student ID; find index; print display() or "Student Not Found."
        if (studentCount == 0) System.out.println("No students to search.");
        else {
    ...
    public void calculateAverage() {
        // average of students[0..studentCount); printf "Average Marks : %.2f%n"
        double sum = 0;
        for (int i = 0; i < studentCount; i++) {
